...
def normal_inv_gamma(alpha, beta, delta, gamma, mu, sigma):
    ...
    gammaFunction = gamma_function(alpha)

    normInvGamma = (np.sqrt(gamma)/sigma*np.sqrt(2*np.pi))*((beta**alpha)/gammaFunction)*((1/sigma**2)**(alpha+1))*np.exp((-2*beta + gamma*(delta - mu)**2)/2*sigma**2)
